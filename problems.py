...
# class HartmannReversed(AugmentedHartmann):
#     def __init__(self, noise_std: Optional[float] = None):
#         super().__init__(noise_std=noise_std)
#         self._max_ = 3.322367993160791
#         self._min_ = 0.0
...
#         H = (H - self._min_)/(self._max_-self._min_)  # normalize
#         H = H.to(self.device)
#         return torch.where(torch.tensor(is_primary_source(X), device=self.device), H  + self.noise_std/10) * torch.randn_like(H, device=self.device), H + (self.noise_std/10) * torch.randn_like(H, device=self.device))
