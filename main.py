...
def eval_model(combi, BUDGET,
               save=None, verbose=False, device="cuda"):
    print("Running on",device)

    experiment, method, cost, cond_var, cond_ig, mogp, repseed, ite = combi
    _, seed = repseed
    ndim, problem, bounds, fmax, list_fidelities, ninits = get_problem_settings(experiment, cost, device=device)
    m = list_fidelities[-1]
    lambda_m = x_obj_cost(torch.ones(ndim+1), cost_model, problem, ndim)[2].item()
    torch.manual_seed(seed)
    MFBO = mfbo(ndim, list_fidelities, bounds, cost_model, cost_aware_utility, IG_COST, IG_INV_COST, mogp, device=device)
    train_x_init, train_obj_init = generate_initial_data(problem, ninits, bounds, ndim, list_fidelities, seed=seed, method="latinhypercube", device=device)
    ...
    if algo=='MF' or algo=='SF':
        ...
        while np.floor(budgetleft/lambda_m) >= 1:
            ...
            budgetleft -= cost.item()
            sr = simple_regret(train_x, problem(torch.cat([train_x[:,:-1],torch.ones(train_x.size(0),1, device=train_x.device)],1)), fmax)
...
def parallel_eval(combi, BUDGET, save, verbose, x, device="cuda"):
