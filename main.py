...
torch.set_default_tensor_type(
    torch.DoubleTensor
)  # otherwise mixed floats and doubles cause a lot of headache

warnings.filterwarnings(
    "ignore", category=UserWarning
)  # damn torch triangular matrices warning
...
def eval_model(combi, BUDGET, save=None, verbose=False, device="cpu"):
    print("Running on", device)

    experiment, method, cost, cond_var, cond_ig, mogp, repseed, ite = combi
    print(method)
    _, seed = repseed
    ndim, problem, bounds, fmax, list_fidelities, ninits = get_problem_settings(
        experiment, cost, device=device
    )
    m = list_fidelities[-1]
    ...
    train_x_init, train_obj_init = generate_initial_data(
        problem,
        ninits,
        bounds,
        ndim,
    ...
    if algo == "rMF":
        ...
        train_x_sf, train_obj_sf = (
            torch.clone(train_x)[is_primary_source(train_x)],
            torch.clone(train_obj)[is_primary_source(train_x)],
        )
        ...
        while np.floor(budgetleft / lambda_m) >= 2:
                ...
                if is_primary_source(x_mf):
                    ...
                    while not condition2 and len(iss_left) > 0:
                        ...
                        if IG_l / cost_l > c2:
                            ...
                            iss_left.remove(l)
                            if len(iss_left) > 0:
                                x_mf = MFBO.optimize_alpha(
                                    model_mf, "MF-" + acquisitionfunc, list(iss_left)
                                )

            if verbose:
                print("sigma_mf(x_sf,m) = " + str(sigma))
            if condition1 and condition2:
                ...
                if verbose:
                    ...
                    print(
                        "pseudo-query value "
                        + str(model_mf.posterior(x_sf).mean.item())
                    )
                    print(
                        "true pseudo-query value "
                        + str(x_obj_cost(x_sf, cost_model, problem, ndim)[1].item())
                    )
                ...
                train_x_sf, train_obj_sf = augment_data(
                    x_sf, torch.tensor([0]).unsqueeze(0), train_x_sf, train_obj_sf
                )
                ...
                train_x, train_obj, train_x_sf, train_obj_sf = augment_data(
                    x_sf, new_obj, train_x, train_obj, train_x_sf, train_obj_sf
                )
            ...
            ir = inference_regret(
                MFBO,
                MFBO.optimal_irmodel(model_sf, model_mf, train_x, train_obj),
                fmax,
                cost_model,
                problem,
                ndim,
        ...
        x_last = MFBO.get_recommendation(
            MFBO.optimal_irmodel(model_sf, model_mf, train_x, train_obj)
        )
        if any(
            [
                torch.allclose(x_last, train_x_sf[i, :])
                for i in range(train_x_sf.shape[0])
            ]
        ):
        ...
        budgetleft -= cost
        train_x, train_obj, train_x_sf, train_obj_sf = augment_data(
            x_last, new_obj, train_x, train_obj, train_x_sf, train_obj_sf
        )
        model_mf = MFBO.update_model(train_x, train_obj)
        model_sf = MFBO.update_model(train_x_sf, train_obj_sf)
        """Regrets"""
    ...
    if algo == "MF" or algo == "SF":
        if algo == "SF":
            train_x, train_obj = (
                torch.clone(train_x_init)[is_primary_source(train_x_init)],
                torch.clone(train_obj_init)[is_primary_source(train_x_init)],
            )
            train_x_init_save = (
                torch.clone(train_x_init)[is_primary_source(train_x_init)],
            )
            train_obj_init_save = torch.clone(train_obj_init)[
                is_primary_source(train_x_init)
            ]
        if algo == "MF":
            train_x, train_obj = torch.clone(train_x_init), torch.clone(train_obj_init)
        budgetleft = BUDGET
            ...
            sr = simple_regret(
                ...
                problem(
                    torch.cat(
                        [
                            train_x[:, :-1],
                            torch.ones(train_x.size(0), 1, device=train_x.device),
                        ],
                        1,
                    )
                ),
    ...
    sr_init = simple_regret(
        train_x_init[: sum(ninits)], train_obj_init[: sum(ninits)], fmax
    )
...
def parallel_eval(combi, BUDGET, save, verbose, x, device="cpu"):
    return eval_model(combi[x], BUDGET, save, verbose, device)
