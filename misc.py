...
def parser_bo():
    ...
    parser.add_argument(
        "-se", "--seed", default=1, help="int, random seed", type=int
    )
    parser.add_argument(
        "-s", "--save", default='./', type=str, help="Save the results on a dict."
    )
    parser.add_argument(
...
def get_problem_settings(experiment: str = "Hartmann", cost: float = .2, device='cuda'):
    ...
    if experiment=="Hartmann":
        ndim = 6
        problem = Hartmann(noise_std=.05).to(**tkwargs)
...
def generate_initial_data(problem, ninits, bounds, ndim, list_fidelities, seed=None, method="random", device="cuda"):
    tkwargs['device'] = device
    ...
    if method=="random":
        train_x = torch.rand(ninit, bounds_.size(1), **tkwargs)
    elif method=="latinhypercube":
        engine = LatinHypercube(d=ndim,seed=seed)
        train_x = torch.tensor(engine.random(n=ninits[0]), **tkwargs)
        for i in range(len(list_fidelities)-1):
            j = i + 1
            train_x = torch.cat((train_x, torch.tensor(engine.random(n=ninits[j]), **tkwargs)), dim=0)
    else:
        raise ValueError("Invalid initial training data sampling method")
    train_x = bounds_[0] + (bounds_[1] - bounds_[0]) * train_x
    train_fid = torch.t(torch.tensor([sum([[list_fidelities[i] for n in range(ninits[i])] for i in range(len(list_fidelities))],[])], **tkwargs))
